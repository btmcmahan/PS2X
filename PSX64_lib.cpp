...
bool PSX64::NewButtonState() {
   return ((last_buttons ^ buttons) > 0);

}

bool PSX64::NewButtonState(unsigned int button) {
  return (((last_buttons ^ buttons) & button) > 0);
}

bool PSX64::ButtonPressed(unsigned int button) {
  return(NewButtonState(button) & Button(button));
}

bool PSX64::ButtonReleased(unsigned int button) {
  return((NewButtonState(button)) & ((~last_buttons & button) > 0));
}
  
bool PSX64::Button(uint16_t button) {
   return ((~buttons & button) > 0);
}

unsigned int PSX64::ButtonDataByte() {
   return (~buttons);
}

uint8_t PSX64::Analog(uint8_t button) {
  return PS2data[button];
}
unsigned char PSX64::_gamepad_shiftinout (char uint8_t) {
   unsigned char tmp = 0;
   for(i=0;i<8;i++) {
...
uint8_t PSX64::config_gamepad(uint8_t clk, uint8_t cmd, uint8_t att, uint8_t dat) {
...
void PSX64::sendCommandString(uint8_t string[], uint8_t len) {
...
uint8_t PSX64::maskToBitNum(uint8_t mask) {
    for (int y = 0; y < 8; y++)
    {
      if(CHK(mask,y))
        return y;
